...
const JobAction: React.FC = () => {
  ...
  const processNextChunk = async (startIndex: number, totalKeywords: number) => {
    try {
      if (isCancelled) {
      ...
      const response = await axiosClient.post(`/api/keywords/check?startIndex=${startIndex}`);
      const { hasMore, nextIndex, processedKeywords, results, cancelled, totalKeywords: total } = response.data;

      // Update progress
      const progressPercentage = Math.round((processedKeywords / totalKeywords) * 100);
  ...
  const handleCheckKeywords = useCallback(async () => {
    try {
      ...
      const initialResponse = await axiosClient.post('/api/keywords/check?startIndex=0');
