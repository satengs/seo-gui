...
import { ChevronRight, ChevronDown, Calendar, Link as LinkIcon, Search, Globe, Bot } from 'lucide-react';
...
const JsonViewer: React.FC<JsonViewerProps> = ({
    ...
    const [expandedHistoricalItems, setExpandedHistoricalItems] = useState<Record<string, boolean>>({});
    ...
    const getKeyIcon = useCallback((key: string) => {
        switch (key.toLowerCase()) {
            case 'keyworddata':
                return <Search className="h-4 w-4 text-blue-700" />;
            case 'data':
                return <Search className="h-4 w-4 text-blue-700" />;
            case 'ai_overview':
    ...
    const renderHistoricalData = useCallback((date: string, data: any) => {
        ...
        if (typeof inputData === 'object' && inputData !== null) {
            ...
            for (const [key, value] of Object.entries(inputData)) {
                if (key === 'keywordData' && typeof value === 'object') {
                    ...
                    processed[key] = value instanceof Map ? Object.fromEntries(value) : value;
                }
            }
            return processed;
        }
    ...
    }, [data, level, expandedKeys, renderValue, renderHistoricalData, processData, getKeyIcon]);
